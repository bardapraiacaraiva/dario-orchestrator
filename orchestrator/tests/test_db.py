...
class TestTaskLifecycle:
    def test_create_task(self, test_db):
        result = test_db.create_task(_task("X-001", "Test task", skill="dario-brand"))
    ...
    def test_assign_task_success(self, test_db):
        test_db.create_task(_task("X-001", "Test", skill="dario-brand"))
    ...
    def test_checkout_requires_assignee(self, test_db):
        test_db.create_task(_task("X-001", "Test"))
...
class TestBudget:
    ...
    def test_budget_accumulates(self, test_db):
        for i in range(3):
            test_db.create_task(_task(f"X-{i}", "Test"))
