...
class VulnScannerRegistry:
    ...
    def get_profile(self, name: str) -> ScanProfile | None: return self._profiles.get(name)
    ...
    def record_result(self, profile: str, result: dict):
        if profile in self._profiles:
            self._profiles[profile].last_run = datetime.now(UTC).isoformat()
    ...
    def check_output(self, output: str) -> dict:
        ...
        for domain, canary in self._canaries.items():
            if canary in output:
                ...
                self._alerts.append({"domain": domain, "timestamp": datetime.now(UTC).isoformat()})
        return {"clean": len(found) == 0, "leaks": found, "total_alerts": len(self._alerts)}
    def get_canary(self, domain: str) -> str | None: return self._canaries.get(domain)
