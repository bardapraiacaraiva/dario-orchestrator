---
name: dario-ios-hig
description: iOS Human Interface Guidelines design skill for native iPhone/iPad app interfaces. Scores and guides SwiftUI/UIKit design to 10/10 Apple HIG compliance. Covers layout, typography, color, navigation, controls, accessibility, icons, gestures, haptics, widgets, privacy, and system integration. Triggers on "ios app", "iphone interface", "swiftUI design", "HIG compliance", "apple design", "ios layout", "dark mode ios", "tab bar", "navigation bar", "SF Symbols", "dynamic type", "voiceover", "ios accessibility", "app icon design", "ios widget", "live activity", "app clip".
license: MIT
---
...
## Red Flags
- Never deviate from Apple HIG conventions (tab bars, navigation stacks, system gestures) without strong, documented justification — non-standard patterns feel foreign to iOS users and trigger app review rejections
- Never skip accessibility (VoiceOver, Dynamic Type, contrast ratios) — inaccessible apps exclude real users, violate App Store guidelines, and fail EAA 2025 compliance requirements
- Always test on real physical devices (minimum: smallest and largest screen sizes) — the Simulator misses haptics, safe area edge cases, real-world performance, and hardware-specific layout issues
